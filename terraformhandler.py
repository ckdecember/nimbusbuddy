...
class TerraformHandler:
    ...
    def __init__(
        ...
        targetRegion,
        ami,
        mainConfigFileName="cloudbuddy-main.tf",
        variableFileName="cloudbuddy-var.tf",
    ):
    ...
    def terraformDump(self):
        ...
        for data in self.resourceDictList["vpc"]:
            if data["IsDefault"]:
                ...
                tfcode += self.resourceOutput(
                    "vpc", data["VpcId"], data["VpcId"], data["Tags"], data["VpcId"]
                )
        fp.write(tfcode)
        ...
        for data in self.resourceDictList["subnet"]:
            if data["VpcId"] == vpcForDefaultSubnet:
                continue
            tfcode += self.resourceOutput(
                "subnet", data["VpcId"], data["SubnetId"], data["Tags"], data["VpcId"]
            )
        fp.write(tfcode)
        ...
        for data in self.resourceDictList["instance"]:
            ...
            if self.amiOverride:
                ...
                tfcode = self.resourceInstanceOutput(
                    data["InstanceId"],
                    data["InstanceType"],
                    self.amiOverride,
                    data["SubnetId"],
        ...
        for data in self.resourceDictList["vpc"]:
            ...
            tfcode = self.variableOutput(
                "vpc", data["VpcId"], data["CidrBlock"], tagValue
            )
            fp.write(tfcode)
        ...
        for data in self.resourceDictList["subnet"]:
            ...
            if "Tags" in data:
                tagValue = data["Tags"]
            tfcode = self.variableOutput(
                "subnet", data["SubnetId"], data["CidrBlock"], tagValue
            )
            fp.write(tfcode)
    ...
    def variableOutput(self, resourceType, varName, typeValue, description):
        ...
        if resourceType == "vpc":
            variableStr = """variable "var_{varName}" {{
                ...
                varName=varName, typeValue=typeValue, tags=description
            )
        elif resourceType == "subnet":
            variableStr = """variable "var_{varName}" {{
                description = "{tags}"
    ...
    def variableInstanceOutput(self, resourceType, varName, typeValue, description):
        ...
        variableStr = """variable "{prefix}_{varName}" {{
            ...
            prefix="ami", varName=varName, tags=tags
        )
        return variableStr
